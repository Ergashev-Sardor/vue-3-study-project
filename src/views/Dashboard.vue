  ...
  <v-card flat>
    ...
    <v-card-text>
      ...
      >
        <template #item.completed="{ item }">
          <v-checkbox
            v-model="item.selectable.completed"
            :ripple="false"
            :label="item.selectable.completed ? 'Completed' : 'Not completed'"
            readonly
          ></v-checkbox>
        </template>
    ...
    <v-dialog v-model="dialogs.view" width="35%">
      <v-card>
        ...
        <v-card-text>
          ...
          ></v-text-field>
          <v-checkbox
            v-model="selectedItem.completed"
            :ripple="false"
            :label="selectedItem.completed ? 'Completed' : 'Not completed'"
            readonly
          ></v-checkbox>
        </v-card-text>
        <v-card-actions>
    ...
    <v-dialog v-model="dialogs.edit" width="35%">
      <v-card>
        ...
        <v-card-actions>
          <v-btn class="bg-success" block @click="onUpdateTodo">Update todo</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { title: 'completed', key: 'completed', align: 'start' },
  ...
  methods: {
    ...
    onUpdateTodo() {
      ...
      fetch('https://jsonplaceholder.typicode.com/todos/' + this.selectedItem.id, {
        method: 'PUT',
        ...
        .then((json) => {
          ...
          this.serverItems[index].completed = this.selectedItem.completed

          this.snackbar.text = `Todo with id ${this.selectedItem.id} successufuly update`
          this.snackbar.model = true
        })
        .catch(() => {
          this.snackbar.text = `Somethings went wrong on update todos with id ${this.selectedItem.id}`
          this.snackbar.model = true
        })
        .finally(() => {
          this.loading = false
    ...
    deleteItem(id) {
      ...
      fetch('https://jsonplaceholder.typicode.com/todos/' + id, {
        ...
        .catch(() => {
          this.snackbar.text = `Somethings went wrong on delete todos with id ${this.selectedItem.id}`
          this.snackbar.model = true
        })
        .finally(() => {
          this.loading = false
